...
class LFAQ_Agent(Agent):
    def __init__(self, agent_id, neighbors, state_set, action_set, gamma=0.95, beta=10**(-3)):
    ...
    def update_q(self, state, reward):
        ...
        alpha = 1/(10+0.01*self.n_strategy[self.prev_action]) #先行研究に準じたalpha
        self.reward_lst.append(reward)

        xi = softmax_boltzman(self.q_table[state])[self.prev_action] #action選択される確率
        fa_val = np.min(1, self.beta/xi)

        #q tableの更新
        self.q_table[self.c_st][a] += fa_val*alpha*(reward+self.gamma*np.nanman(np.array(self.q_table[state], dtype=np.float64))-self.q_table[self.c_st][a])
